...
cargaespera::cargaespera(QString asiento) : QDialog() 
  {
    ui.setupUi(this);
    CustomSqlModelC *elmodelo = new CustomSqlModelC;
    model = elmodelo;
    QString cadena;
    cadena="SELECT asiento,usuario,cuenta,concepto,debe,haber,documento,ci "
            "from borrador";
	if (asiento.length()>0){
		cadena+=" WHERE asiento=";
		cadena+=asiento;
	}
	cadena+=" order by asiento,cuenta";
...
{
 ui.elborrador->~QTableView();
 model->~CustomSqlModelC();
}
